...
//设置SDA输出模式
#define SDA_OUT()   {                                                  \
                        DL_GPIO_initDigitalOutput(IMU_IMU_SDA_IOMUX);     \
                        DL_GPIO_setPins(IMU_PORT, IMU_IMU_SDA_PIN);      \
                        DL_GPIO_enableOutput(IMU_PORT, IMU_IMU_SDA_PIN); \
                    }
//设置SDA输入模式
#define SDA_IN()    { DL_GPIO_initDigitalInput(IMU_IMU_SDA_IOMUX); }

//获取SDA引脚的电平变化
#define SDA_GET()   ( ( ( DL_GPIO_readPins(IMU_PORT,IMU_IMU_SDA_PIN) & IMU_IMU_SDA_PIN ) > 0 ) ? 1 : 0 )
//SDA与SCL输出
#define SDA(x)      ( (x) ? (DL_GPIO_setPins(IMU_PORT,IMU_IMU_SDA_PIN)) : (DL_GPIO_clearPins(IMU_PORT,IMU_IMU_SDA_PIN)) )
#define SCL(x)      ( (x) ? (DL_GPIO_setPins(IMU_PORT,IMU_IMU_SCL_PIN)) : (DL_GPIO_clearPins(IMU_PORT,IMU_IMU_SCL_PIN)) )
