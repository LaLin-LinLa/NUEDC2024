...
char MPU6050_WriteReg(uint8_t addr,uint8_t regaddr,uint8_t num,uint8_t *regdata)
{
    uint16_t i = 0;
        IICMPU_Start();
        Send_Byte((addr<<1)|0);
        if( I2C_WaitAck() == 1 ) {IICMPU_Stop();return 1;}
        Send_Byte(regaddr);
        if( I2C_WaitAck() == 1 ) {IICMPU_Stop();return 2;}
    
        for(i=0;i<num;i++)
    {
        Send_Byte(regdata[i]);
        if( I2C_WaitAck() == 1 ) {IICMPU_Stop();return (3+i);}
    }        
        IICMPU_Stop();
    return 0;
}
...
char MPU6050_ReadData(uint8_t addr, uint8_t regaddr,uint8_t num,uint8_t* Read)
{
        ...
        IICMPU_Start();
        Send_Byte((addr<<1)|1);
        if( I2C_WaitAck() == 1 ) {IICMPU_Stop();return 3;}
